          ...
          <aside className="w-full md:w-64 shrink-0">
            <div className="bg-white rounded-2xl p-5 border border-ink-100 shadow-soft mb-4">
              <div className="flex items-center gap-3 mb-1">
                <div className="w-12 h-12 rounded-full bg-brand-100 text-brand-800 flex items-center justify-center font-bold text-lg">
              ...
              {tabs.map((tab) => (
                ...
                >
                  ...
                  {tab.id === "wishlist" && wishlistItems.length > 0 && (
                    <span className="ml-auto text-[10px] bg-brand-200 text-brand-900 px-1.5 py-0.5 rounded-md font-bold">
                  ...
                  <div className="bg-white rounded-2xl p-6 border border-ink-100 shadow-soft space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="px-4 py-2.5 bg-ink-50 rounded-xl text-sm text-ink-900 font-medium">{user.username}</div>
                      </div>
                      <div>
                        <label className="text-xs text-ink-500 mb-1 block">Phone</label>
                        <div className="px-4 py-2.5 bg-ink-50 rounded-xl text-sm text-ink-900 font-medium">{user.phone}</div>
                      </div>
                      <div>
                        <label className="text-xs text-ink-500 mb-1 block">Email</label>
                        <div className="px-4 py-2.5 bg-ink-50 rounded-xl text-sm text-ink-900 font-medium">{user.email || "—"}</div>
                      </div>
                      <div>
                        <label className="text-xs text-ink-500 mb-1 block">Role</label>
                        <div className="px-4 py-2.5 bg-ink-50 rounded-xl text-sm text-ink-900 font-medium capitalize">{user.role}</div>
                      </div>
                    </div>
                  </div>
                </motion.div>
              ...
              {activeTab === "wishlist" && (
                <motion.div key="wishlist" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                  ...
                  {wishlistItems.length === 0 ? (
                    <div className="bg-white rounded-2xl p-8 border border-ink-100 text-center text-ink-500 text-sm">
                      No items saved yet. <Link href="/shop" className="text-brand-700 font-semibold hover:underline">Browse shop</Link>
                    </div>
                  ) : (
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {wishlistItems.map((w) => (
                        <Link key={w.product.id} href={`/product/${w.product.slug}`} className="group block bg-white rounded-2xl border border-ink-100 overflow-hidden shadow-soft hover:shadow-premium transition-shadow">
                          ...
                          <div className="p-3">
                            <div className="text-sm font-semibold text-ink-900 group-hover:text-brand-700 transition-colors">{w.product.name}</div>
                          </div>
                        </Link>
                      ))}
                    </div>
                    ...
                    {userMessages.length === 0 ? (
                      ...
                      userMessages.map((m) => (
                        <div key={m.id} className={`p-4 flex gap-3 ${m.sender === "admin" ? "bg-brand-50/50" : ""}`}>
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${m.sender === "admin" ? "bg-brand-200 text-brand-800" : "bg-ink-100 text-ink-600"}`}>
                            {m.sender === "admin" ? "A" : "U"}
                          </div>
                          <div className="flex-1">
                            <div className="flex items-center justify-between mb-0.5">
                              <span className="text-xs font-semibold text-ink-800">{m.sender === "admin" ? "STARBOY Support" : "You"}</span>
                              <span className="text-[10px] text-ink-400">{new Date(m.createdAt).toLocaleDateString()}</span>
                            </div>
                            <p className="text-sm text-ink-700">{m.text}</p>
                            {m.productName && (
                              <span className="inline-block mt-1.5 text-[10px] font-semibold text-brand-700 bg-brand-50 px-2 py-0.5 rounded-md">
                                {m.productName}
                              </span>
                            )}
                          </div>
              ...
              {activeTab === "settings" && (
                <motion.div key="settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                  ...
                  <div className="bg-white rounded-2xl p-6 border border-ink-100 shadow-soft space-y-4">
                    <div>
                      <label className="text-xs text-ink-500 mb-1 block">Username</label>
                      <input defaultValue={user.username} className="w-full px-4 py-2.5 bg-ink-50 border border-ink-200 rounded-xl text-sm outline-none focus:border-brand-500" />
                    </div>
                    <div>
                      <label className="text-xs text-ink-500 mb-1 block">Email</label>
                      <input defaultValue={user.email || ""} className="w-full px-4 py-2.5 bg-ink-50 border border-ink-200 rounded-xl text-sm outline-none focus:border-brand-500" />
                    </div>
                    <button className="px-5 py-2.5 bg-ink-950 text-brand-300 text-sm font-semibold rounded-xl hover:bg-brand-700 hover:text-white transition-colors">
                      Save Changes
                    </button>
                  </div>
                </motion.div>
