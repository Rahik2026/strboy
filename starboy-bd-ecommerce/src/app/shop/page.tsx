...
export default function ShopPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "sale") {
      list = list.filter((p) => p.offerPrice && p.offerPrice < p.originalPrice);
    ...
    if (sort === "new") {
      list = list.reverse(); // mock: reversed as new
    } else if (sort === "price_low") {
      list.sort((a, b) => (a.offerPrice ?? a.originalPrice) - (b.offerPrice ?? b.originalPrice));
    } else if (sort === "price_high") {
      list.sort((a, b) => (b.offerPrice ?? b.originalPrice) - (a.offerPrice ?? a.originalPrice));
    }
    ...
    return list;
  }, [categorySlug, filter, sort, priceRange]);
            ...
            <div className="fixed inset-0 z-50 bg-ink-950/50 backdrop-blur-sm lg:hidden" onClick={() => setMobileFilterOpen(false)}>
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold mb-2">Price Max: ৳{priceRange[1]}</h4>
                    <input
                      type="range"
                      min={0}
                      max={10000}
          ...
          <div className="flex-1">
            ...
            {filtered.length === 0 && (
              <div className="text-center py-20">
                <p className="text-ink-500 text-sm">No products found for this filter.</p>
              </div>
            )}
          </div>
        </div>
