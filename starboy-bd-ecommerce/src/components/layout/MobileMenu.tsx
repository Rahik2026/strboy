      ...
      {open && (
        <>
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 bg-ink-950/50 z-[70] backdrop-blur-sm"
          />
          <motion.div
            initial={{ x: "-100%" }}
            animate={{ x: 0 }}
            ...
            className="fixed top-0 left-0 bottom-0 w-[85vw] max-w-sm bg-white z-[80] shadow-2xl flex flex-col"
          >
            <div className="flex items-center justify-between p-4 border-b border-ink-100">
              <span className="font-display text-lg font-bold text-ink-950">STARBOY BD</span>
              <button onClick={onClose} className="p-2 text-ink-600 hover:text-ink-950">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-auto py-2">
              <div className="px-4 py-2">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search..."
                    className="w-full pl-9 pr-4 py-2.5 bg-ink-50 rounded-xl text-sm outline-none focus:ring-2 focus:ring-brand-200"
                  />
                </div>
              </div>
              ...
              <div className="px-4 mt-4 space-y-1">
                ...
                {[
                  ...
                  { label: "Men's Collection", href: "/shop?category=mens-collection" },
                  ...
                  <Link
                    ...
                    href={item.href}
                    onClick={onClose}
                    className="flex items-center justify-between px-3 py-3 rounded-xl text-ink-800 hover:bg-brand-50 transition-colors"
                  >
              ...
              <div className="px-4 mt-6 space-y-1">
                ...
                {categories.map((cat) => (
                  <Link
                    ...
                    onClick={onClose}
                    className="flex items-center gap-3 px-3 py-3 rounded-xl text-ink-800 hover:bg-brand-50 transition-colors"
                  >
                    <div
              ...
              {user ? (
                <div className="space-y-2">
                  <Link
                    href="/dashboard"
                    onClick={onClose}
                    className="flex items-center gap-2 w-full px-4 py-3 rounded-xl bg-ink-950 text-brand-300 text-sm font-semibold justify-center"
                  >
                    <User className="w-4 h-4" /> My Account
                  </Link>
                  <button
                    onClick={() => { logout(); onClose(); }}
                    className="flex items-center gap-2 w-full px-4 py-3 rounded-xl border border-ink-200 text-ink-700 text-sm font-medium justify-center"
                  >
                    Sign Out
                  </button>
                </div>
